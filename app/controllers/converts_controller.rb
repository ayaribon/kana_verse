    ...
    words.each do |word|
      ...
      characters.each_with_index do |char, index|
        ...
        if char == "っ"
          ...
          # （1）と（2）の条件に該当する場合、変換をスキップ
          next
        elsif %w[あ い う え お].include?(char) && prev_char && hiragana_rows[prev_char] == hiragana_rows[char]
          # （3）ひらがなが「あ、い、う、え、お」のどれかで、ひとつ前のひらがなが同じ段の場合、変換をスキップ
          next
        elsif index == characters.length - 1
          # 単語の最後のひらがなは短のラオス文字になる（4）
          converted_word << (hiragana_to_lao_short[char] || char)
        else
          if next_char == "っ"
              converted_word << (hiragana_to_lao_short[char] || char)
            # 「っ」の前のひらがなは短のラオス文字になる
          elsif next_char == "ん"
            # 「ん」の前のひらがなは特のラオス文字になる
